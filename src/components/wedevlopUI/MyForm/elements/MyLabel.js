...
const MyLabel = props => {
  ...
  return (
    <div>
      <div
        style={{
          ...labelContainerStyles.style,
          ...labelContainerStyles.addStyle
        }}
        className={`${labelContainerStyles.className} ${labelContainerStyles.addClassName}`}
      >
        {labelExternalComponent1 !== null &&
          labelExternalComponent1 !== undefined && (
            <div>{labelExternalComponent1}</div>
          )}
        <label
          style={{ ...labelStyles.style, ...labelStyles.addStyle }}
          className={`${labelStyles.className} ${labelStyles.addClassName}`}
        >
          {useIntlMsgID === true ? <IntlMessages id={intlMsgID} /> : text}
        </label>
        {labelExternalComponent2 !== null &&
          labelExternalComponent2 !== undefined && (
            <div>{labelExternalComponent2}</div>
          )}
      </div>
    </div>
  );
